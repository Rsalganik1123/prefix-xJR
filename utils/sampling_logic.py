...
def generate_sample_sets_stratified_by_bin_old(
    dataset_cfg,
    n_samples,
    n_users,
    n_items,
    ...
    Returns:
      dfs: list of sampled user preference dfs with columns [user_key, Ranked_Items]
      items: list of np arrays of sampled item ids
      users: list of np arrays of sampled user indices (same as your original code)
    """
    user_key = dataset_cfg["dataset"]["keys"]["user_key"]
    ...
    # Build bin -> list(items)
    bin_to_items = {b: sub[item_key].to_numpy() for b, sub in labeled.groupby(bin_col)}
    ...
    # Bin weights: equal over bins by default
    ...
    for seed in range(n_samples):
        ...
        # ============================================================
        # filter rows to sampled items + sort by est_rating_key
        # ============================================================
        item_set = set(sampled_items_arr.tolist())
        
        
        
        sampled_rows = preferences[
            (preferences[user_key].isin(sampled_user_ids)) &
            (preferences[item_key].isin(item_set))
        ].copy()

        sampled_rows = sampled_rows.sort_values(
            by=[user_key, est_rating_key],
            ascending=[True, False]
        )

        ranked_df = (
            sampled_rows
            .groupby(user_key)[item_key]
            .apply(list)
...
def generate_sample_sets_old(dataset_cfg, n_samples, n_users, n_items, all_items, preferences):
    user_key = dataset_cfg['dataset']["keys"]["user_key"]
    item_key = dataset_cfg['dataset']["keys"]["item_key"]
    est_rating_key = dataset_cfg['dataset']["keys"]["est_rating_key"]
    ...
    for seed in range(n_samples):
        ...
        user_idx = np.random.choice(len(all_users), size=n_users, replace=False)
        sampled_users = [all_users[i] for i in user_idx]
        ...
        # Filter rows
        # -------------------------------------------------
        sampled_pref = preferences[
            (preferences[user_key].isin(sampled_users)) &
            (preferences[item_key].isin(item_set))
        ].copy()
        ...
        # -------------------------------------------------
        sampled_pref = sampled_pref.sort_values(
            by=[user_key, est_rating_key],
            ascending=[True, False]
        )

        # -------------------------------------------------
        # Rebuild ranked lists per user
        # -------------------------------------------------
        ranked_df = (
            sampled_pref
            .groupby(user_key)[item_key]
